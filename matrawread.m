...
% 203 to specify the version you wish to call.
%
% USAGE:
...
%                false)
% renew:         specify whether to ignore the temporary .pgm buffer and
%                re-generate a new .pgm file by calling dcraw.exe in the
%                cmd. Setting 'renew' to false and 'keeppgm' to true would
%                highly reduce the processing time if the matrawread()
%                would be executed several times with the same parameters.
%                (default = true)
...
for i = 1:numel(folder_contents)
    ...
    if ~exist(pgm_file, 'file') || param.renew
...
parser.addParameter('renew', true, @(x)islogical(x));
...
% make format pretty
if param.save == true
    attr_indices = [6, 9, 2, 14, 3, 1, 7, 5, 11, 8, 13, 12, 15, 4, 16];
else
    attr_indices = [6, 9, 2, 14, 3, 1, 7, 5, 11, 8, 13];
end
...
field_name_dict.renew = 'Regenerate the temporary .pgm files';
